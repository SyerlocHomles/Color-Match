...
<style>
    ...
    .stButton > button {
        ...
        height: 35px !important; border-radius: 6px !important;
    }
    .action-gap { margin-bottom: 30px; }
</style>
...
if not st.session_state.game_active:
    ...
    <div class="desc-text">
        ...
        2. Klik tombol 'Ganti' untuk mengubah warna kartu.<br>
    ...
    # Baris Kartu
    c1 = st.columns(st.session_state.max_k)
    for i in range(st.session_state.max_k):
        c1[i].markdown(f'<div class="card-slot" style="background-color:{WARNA_HEX[st.session_state.guesses[i]]};"></div>', unsafe_allow_html=True)
    
    # Baris Tombol Ganti
    c2 = st.columns(st.session_state.max_k)
    for i in range(st.session_state.max_k):
        c2[i].button("Ganti", key=f"g_{i}", on_click=ganti_warna, args=(i,))
    ...
    if st.button("CEK JAWABAN (OK) ✅", use_container_width=True):
        if "Kosong" not in st.session_state.guesses:
            ...
            if st.session_state.guesses == st.session_state.target:
                st.balloons(); st.success("🎉 JACKPOT! MENANG!")
